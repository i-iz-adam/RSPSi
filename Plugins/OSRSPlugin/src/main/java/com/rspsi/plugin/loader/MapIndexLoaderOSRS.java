...
public class MapIndexLoaderOSRS extends MapIndexLoader {
	...
	public String getGroupName(int hash, MapType type) {
		int index = IntStream.range(0, mapHashes.length)
				...
				.orElse(-1);
		if(index >= 0) {
			return type == MapType.LANDSCAPE ? "m" + groupName[index] : "l" + groupName[index];
		}
	...
	public void init(Index mapIndex) {
		...
		for (int i = 0; i < 32768; i++) {	
			...
			RegionData data = new RegionData(i, map != null ? map.getId() : -1, land != null ? land.getId() : -1,x + "_" + y);
